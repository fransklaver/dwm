...
attachclients(Monitor *m) {
	...
	for (c = m->cl->clients; c; c = c->next) {
		if (ISVISIBLE(c, m)) {
			/* if client is aso visible on other tags that are displayed on
...
{
	Client *c, *p = NULL, *pc = NULL, *i;

	if (arg->i > 0) {
		/* fiend the client after selmon->sel */
	...
	/* swap c and selmon->sel clients in the selmon-clients list */
	if (c && c != selmon->sel) {
